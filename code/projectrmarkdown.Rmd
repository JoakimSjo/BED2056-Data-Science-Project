...
Applying this method, the group started with a workshop deciding the project and raising questions we wanted to answer with this project. Data sets were then gathered, understood, and reviewed to ensure that they were of the right quality and relevant to the project. \ 

The group then started a process where the data was modified to be ready for analysis. In this stage, we had to alter the population data sets to create a new data set grouped by race and state only such that the total population for each race was obtained in each state. Also, we replaced numerical representations of states, e.g., "36" to "NY" for "New York" to make it more intuitive when examining the data set. A year column was appended on the fatality data set, and the date and race columns were altered to reflect the population data set in terms of the mentioned column values. \ 

With the data sets ready for analysis, the group started to create data frames that held data to answer the questions mentioned in the report's introduction. The group explored the different ways to showcase the data through pie charts, bar and graph plots. From the plots, the group draws different conclusions to the mentioned questions. \ 

A presentation was held for the entire class showcasing the results and how the results were obtained. The group got feedback on questions we could ask to discover unknown patterns from the data. E.g., one idea from the feedback was to look at how the fatalities growth rates changed based on protests and president election results. \ 

The group argues that this method is reliable because it challenges the group to define concrete questions to answer early in the process and encourage them to understand the data sets used in the project. Besides, the process is an iterative process that enables seeking feedback and takes action from the input. \ 
